...
(function () {
    ...
    UI.addClass({
         ...
         * @example
         * ```js
         * $('#idInputTime').attr('data-loformatter', 'time').loformatter();
         * $('#idInputMoney').attr('data-loformatter', 'money').loformatter();
         * $('#idInputPhone').attr('data-loformatter', 'phone').loformatter();
         * $('#idInputDate').attr('data-loformatter', 'date').loformatter();
         *
         * $('#loformatter-custom').loformatter({
         *     pattern: "custom",
        ...
        return function () {
            ...
            var formatterEvent = {
                ...
                keydown: function keydown(opts, optIdx, e) {
                    ...
                    if (isStop) luplino.util.stopEvent(e);
                },
                /* 키 업 이벤트에서 패턴을 적용 */
            ...
            var bindFormatterTarget = function bindFormatterTarget(opts, optIdx) {
                if (!opts.pattern) {
                    if (opts.$target.get(0).tagName == "INPUT") {
                        opts.pattern = opts.$target.attr("data-loformatter");
                    } else {
                        opts.pattern = opts.$target.find('input[type="text"]').attr("data-loformatter");
                    }
                    if (!opts.pattern) {
                        console.log(luplino.info.getError("loformatter", "501", "bind"));
                ...
                opts.$input.unbind("focus.loformatter").bind("focus.loformatter", formatterEvent.focus.bind(this, this.queue[optIdx], optIdx));

                opts.$input.unbind("keydown.loformatter").bind("keydown.loformatter", formatterEvent.keydown.bind(this, this.queue[optIdx], optIdx));

                opts.$input.unbind("keyup.loformatter").bind("keyup.loformatter", formatterEvent.keyup.bind(this, this.queue[optIdx], optIdx));

                opts.$input.unbind("blur.loformatter").bind("blur.loformatter", formatterEvent.blur.bind(this, this.queue[optIdx], optIdx));
            ...
            var getQueIdx = function getQueIdx(boundID) {
                ...
                /*
                           if (!U.isString(boundID)) {
                           console.log(luplino.info.getError("loformatter", "402", "getQueIdx"));
                           return;
                           }
                           */
             ...
             * @method loformatter.setConfig
             * @param {Object} config - 클래스 속성값
             * @returns {luplino.ui.formatter}
             * @example
             * ```
             * ```
             */
            ...
            this.bind = function (opts) {
                ...
                if (!opts.target) {
                    console.log(luplino.info.getError("loformatter", "401", "bind"));
                    return this;
                }
                ...
                if (!opts.id) opts.id = opts.$input.data("lo-formatter");

                if (!opts.id) {
                    opts.id = "lo-formatter-" + luplino.getGuid();
                    opts.$input.data("lo-formatter", opts.id);
                }
             ...
             * @method loformatter.formatting
             * @returns {loformatter}
             * @example
             * ```js
             * $('[data-loformatter="time"]').loformatter("formatting"); // 하나만
             * $('[data-loformatter]').loformatter("formatting"); // 모두
             * ```
             */
    ...
    FORMATTER = luplino.ui.formatter;
})();

// luplino.ui.formatter.formatter
(function () {
    var FORMATTER = luplino.ui.formatter;
    var U = luplino.util;
    var TODAY = new Date();
...
luplino.ui.formatter_instance = new luplino.ui.formatter();

jQuery.fn.loformatter = function () {
    return function (config) {
        if (luplino.util.isString(arguments[0])) {
            ...
            switch (methodName) {
                case "formatting":
                    return luplino.ui.formatter_instance.formatting(this);
                    break;

                case "unbind":
                    return luplino.ui.formatter_instance.unbind(this);
                    break;
            ...
            jQuery.each(this, function () {
                ...
                luplino.ui.formatter_instance.bind(config);
            });
        }
        return this;
    };
