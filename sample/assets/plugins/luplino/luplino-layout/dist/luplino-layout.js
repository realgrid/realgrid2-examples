...
(function () {
    ...
    UI.addClass({
        ...
        /**
         * @class lolayout
         * @alias luplino.ui.layout
         * @author jowrney@jowrney.com
         */

        var lolayout = function lolayout() {
            var self = this,
                cfg,
                ENM = {
            ...
            var onStateChanged = function onStateChanged(opts, that) {
                ...
                resizeSplitter = {
                    "on": function on(queIdx, panel, $splitter) {
                        ...
                        jQuery(document.body).bind(ENM["mousemove"] + ".lolayout-" + this.instanceId, function (e) {
                            if (!self.resizer) {

                                self.resizer = jQuery('<div class="lolayout-resizer panel-' + panel.resizerType + '" ondragstart="return false;"></div>');
                                self.resizer.css({
                        ...
                        }).bind(ENM["mouseup"] + ".lolayout-" + this.instanceId, function (e) {
                            resizeSplitter.off.call(self, queIdx, panel, $splitter);
                        }).bind("mouseleave.lolayout-" + this.instanceId, function (e) {
                            resizeSplitter.off.call(self, queIdx, panel, $splitter);
                        });
                    ...
                    "off": function off(queIdx, panel, $splitter) {
                        ...
                        jQuery(document.body).unbind(ENM["mousemove"] + ".lolayout-" + this.instanceId).unbind(ENM["mouseup"] + ".lolayout-" + this.instanceId).unbind("mouseleave.lolayout-" + this.instanceId);
                ...
                bindLayoutTarget = function () {
                    ...
                    var applyLayout = {
                        ...
                        "tab-panel": function tabPanel(queIdx) {
                            ...
                            // make tabLabel
                            item.$target.append(jQuery(luplino.mustache.render(getTabLabesTmpl.call(this, queIdx), item)));
                    ...
                    return function (queIdx) {
                        ...
                        // 부모 컨테이너가 lolayout인지 판단 필요.
                        if (item.$target.parents("[data-lolayout]").get(0)) {
                            hooksResizeLayout.call(this, item.$target.parents("[data-lolayout]").get(0), queIdx);
                        }
                ...
                getQueIdx = function getQueIdx(boundID) {
                    if (!U.isString(boundID)) {
                        boundID = jQuery(boundID).data("data-lolayout-id");
                    }
                    if (!U.isString(boundID)) {
                        //console.log(luplino.info.getError("lolayout", "402", "getQueIdx"));
                        return -1;
                    }
             ...
             * @example
             * ```js
             * jQuery('[data-lolayout="lo1"]').lolayout({
             *     onResize: function () {
             *     }
             * });
             *
             * jQuery('[data-lolayout="lo1"]').lolayout("resize", {
             *     top: {height: 100},
             *     bottom: 100,
             *     left: 100,
             *     right: 100
            ...
            this.init = function () {
                ...
                jQuery(window).bind("resize.lolayout-" + this.instanceId, function () {
                    alignLayoutAll.call(this);
                }.bind(this));
            };

            /**
             * @method lolayout.bind
             * @param {Object} item
            ...
            this.bind = function (item) {
                ...
                if (!item.target) {
                    console.log(luplino.info.getError("lolayout", "401", "bind"));
                    return this;
                }

                item.$target = jQuery(item.target);

                if (!item.id) item.id = item.$target.data("data-lolayout-id");
                if (!item.id) {
                    item.id = 'lolayout-' + luplino.getGuid();
                    item.$target.data("data-lolayout-id", item.id);
                }
                item.name = item.$target.attr("data-lolayout");
                if (item.options) {
            ...
            this.onResize = function (boundID, fn) {
                var queIdx = U.isNumber(boundID) ? boundID : getQueIdx.call(this, boundID);
                if (queIdx === -1) {
                    console.log(luplino.info.getError("lolayout", "402", "onResize"));
                    return;
                }
            ...
            /**
             * @method lolayout.tabOpen
             * @param boundID
             * @param tabIndex
             * @returns {luplino.ui.lolayout}
             */
            this.tabOpen = function () {
                return function (boundID, tabIndex) {
                    var queIdx = U.isNumber(boundID) ? boundID : getQueIdx.call(this, boundID);
                    if (queIdx === -1) {
                        console.log(luplino.info.getError("lolayout", "402", "tabOpen"));
                        return;
                    }
...
luplino.ui.layout_instance = new luplino.ui.layout();

/**
 * lolayout jquery extends
 * @namespace jQueryExtends
 */

/**
 * @method jQueryExtends.lolayout
 * @param {String} methodName
 * @example
 * ```js
 * jQuery('[data-lolayout="lo1"]').lolayout("align");
 * jQuery('[data-lolayout="lo1"]').lolayout("resize");
 * jQuery('[data-lolayout="lo1"]').lolayout("reset");
 * jQuery('[data-lolayout="lo1"]').lolayout("hide");
 * jQuery('[data-lolayout="lo1"]').lolayout("onResize");
 * jQuery('[data-lolayout="lo1"]').lolayout("tabOpen", 1);
 * ```
 */
jQuery.fn.lolayout = function () {
    return function (config) {
        if (luplino.util.isString(arguments[0])) {
            ...
            switch (methodName) {
                case "align":
                    return luplino.ui.layout_instance.align(this, arguments[1]);
                    break;
                case "resize":
                    return luplino.ui.layout_instance.resize(this, arguments[1], arguments[2]);
                    break;
                case "reset":
                    return luplino.ui.layout_instance.reset(this, arguments[1]);
                    break;
                case "hide":
                    return luplino.ui.layout_instance.hide(this, arguments[1]);
                    break;
                case "onResize":
                    return luplino.ui.layout_instance.onResize(this, arguments[1]);
                    break;
                case "tabOpen":
                    return luplino.ui.layout_instance.tabOpen(this, arguments[1]);
                    break;
                case "getActiveTab":
                    return luplino.ui.layout_instance.getActiveTab(this, arguments[1]);
                    break;
                default:
                    return this;
            }
            ...
            jQuery.each(this, function () {
                ...
                luplino.ui.layout_instance.bind(config);
            });
        }
        return this;
    };
