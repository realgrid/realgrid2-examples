...
(function () {
    ...
    UI.addClass({
         ...
         * @example
         * ```js
         * luplino.def.picker.date_leftArrow = '<i class="fa fa-chevron-left"></i>';
         * luplino.def.picker.date_yearTmpl = '%s';
         * luplino.def.picker.date_monthTmpl = '%s';
        ...
        return function () {
            ...
            this.config = {
                ...
                calendar: {
                    ...
                    control: {
                        left: luplino.def.picker.date_leftArrow || '&#x02190',
                        yearTmpl: luplino.def.picker.date_yearTmpl || '%s',
                        monthTmpl: luplino.def.picker.date_monthTmpl || '%s',
                        right: luplino.def.picker.date_rightArrow || '&#x02192',
                        yearFirst: true
                    }
                },
                palette: {}
            ...
            var bindPickerTarget = function () {
                ...
                var pickerType = {
                    ...
                    'color': function color(queIdx, _input) {
                        var item = this.queue[queIdx],
                            ...
                            $colorPreview = item.$target.find('[data-lopicker-color="preview"]');
                ...
                return function (queIdx) {
                    ...
                    if (!item.content) {
                        console.log(luplino.info.getError("lopicker", "501", "bind"));
                        return this;
                    }
                    ...
                    input.unbind('focus.lopicker').unbind('click.lopicker').bind('focus.lopicker', pickerEvent.focus.bind(this, queIdx)).bind('click.lopicker', pickerEvent.click.bind(this, queIdx));

                    item.$target.find('.input-group-addon').unbind('click.lopicker').bind('click.lopicker', pickerEvent.click.bind(this, queIdx));

                    if (item.content.formatter && luplino.ui.formatter) {
                        input.loformatter(item.content.formatter);
                    }
            ...
            var getQueIdx = function getQueIdx(boundID) {
                if (!U.isString(boundID)) {
                    boundID = jQuery(boundID).data("data-lopicker-id");
                }
                if (!U.isString(boundID)) {
                    console.log(luplino.info.getError("lopicker", "402", "getQueIdx"));
                    return;
                }
            ...
            this.bind = function (item) {
                ...
                if (!item.target) {
                    console.log(luplino.info.getError("lopicker", "401", "bind"));
                    return this;
                }
                ...
                if (!item.id) item.id = item.$target.data("data-lopicker-id");

                if (!item.id) {
                    item.id = 'lo-picker-' + luplino.getGuid();
                    item.$target.data("data-lopicker-id", item.id);
                }
            ...
            /**
             * @method lopicker.setContentValue
             * @param {String} boundID
             * @param {Number} inputIndex
             * @param {String} val
             * @returns {lopicker} this
             */
                    ...
                    "date": function date(_item, _inputIndex, _val) {
                        ...
                        if (_inputIndex == 0) {
                            if (values.length > 1 && values[1] !== "") {
                                // 값 검증
                                diffDay = luplino.util.dday(values[1], { today: values[0] });
                                if (diffDay < 0) {
                                    // 다음날짜 달력을 변경합니다.
                                    nextInputValue = _val;
                                } else {}
                            ...
                            if (nextInputValue) {
                                _item.pickerCalendar[1].louiInstance.setSelection([nextInputValue], false).changeMode("d", nextInputValue);
                            ...
                            if (prevInputValue) {
                                _item.pickerCalendar[0].louiInstance.setSelection([prevInputValue], false).changeMode("d", prevInputValue);
                ...
                return function (boundID, inputIndex, val, _option) {
                    ...
                    if (item) {
                        ...
                        if (!item.disableChangeTrigger) {
                            ...
                            var $colorPreview = item.$target.find('[data-lopicker-color="preview"]');
                            if ($colorPreview.get(0)) {
            ...
            /**
             * @method lopicker.getContentValue
             * @param {String} boundID
             * @param {Number} inputIndex
             * @returns {lopicker} this
             */
            ...
            this.open = function () {
                ...
                var pickerContent = {
                    ...
                    'date': function date(queIdx) {
                        ...
                        for (var i = 0; i < item.inputLength; i++) {
                            html.push('<div ' + 'style="width:' + U.cssNumber(item.content.width) + ';float:left;" ' + 'class="lo-picker-content-box" ' + 'data-calendar-target="' + i + '"></div>');
                        ...
                        item.pickerContent.find('[data-calendar-target]').each(function () {
                            ...
                            var idx = this.getAttribute("data-calendar-target"),
                                ...
                                d = luplino.util.date(dValue),
                                dateConvert = {
                                    "year": function year(_d) {
                                        return luplino.util.date(_d, { "return": "yyyy" });
                                    },
                                    "month": function month(_d) {
                                        return luplino.util.date(_d, { "return": "yyyy-MM" });
                                    },
                                    "day": function day(_d) {
                                        return _d;
                                    }
                            ...
                            item.pickerCalendar.push({
                                itemId: item.id,
                                inputIndex: idx,
                                louiInstance: new luplino.ui.calendar(calendarConfig)
                            });
                        });
                    },
                    'secure-num': function secureNum(queIdx) {
                        var item = this.queue[queIdx],
                            html = [];
                        for (var i = 0; i < item.inputLength; i++) {
                            html.push('<div ' + 'style="width:' + U.cssNumber(item.content.width) + ';float:left;" ' + 'class="lo-picker-content-box" ' + 'data-secure-num-target="' + i + '"></div>');
                            if (i < item.inputLength - 1) html.push('<div style="width:' + item.content.margin + 'px;float:left;height: 5px;"></div>');
                        }
                        html.push('<div style="clear:both;"></div>');
                        item.pickerContent.html(html.join(''));
                    ...
                    'keyboard': function keyboard(queIdx) {
                        ...
                        var html = [];
                        for (var i = 0; i < item.inputLength; i++) {
                            html.push('<div ' + 'style="width:' + U.cssNumber(item.content.width) + ';float:left;" ' + 'class="lo-picker-content-box" ' + 'data-keyboard-target="' + i + '"></div>');
                            if (i < item.inputLength - 1) html.push('<div style="width:' + item.content.margin + 'px;float:left;height: 5px;"></div>');
                        }
                        html.push('<div style="clear:both;"></div>');
                        item.pickerContent.html(html.join(''));
                    ...
                    'numpad': function numpad(queIdx) {
                        var item = this.queue[queIdx],
                            html = [];
                        for (var i = 0; i < item.inputLength; i++) {
                            html.push('<div ' + 'style="width:' + U.cssNumber(item.content.width) + ';float:left;" ' + 'class="lo-picker-content-box" ' + 'data-numpad-target="' + i + '"></div>');
                            if (i < item.inputLength - 1) html.push('<div style="width:' + item.content.margin + 'px;float:left;height: 5px;"></div>');
                        }
                        html.push('<div style="clear:both;"></div>');
                        item.pickerContent.html(html.join(''));
                    ...
                    'color': function color(queIdx) {
                        ...
                        for (var i = 0; i < item.inputLength; i++) {
                            html.push('<div ' + 'style="padding: 5px;width:' + U.cssNumber(item.content.width) + ';float:left;" ' + 'class="lo-picker-content-box" ' + 'data-palette-target="' + i + '" data-lopalette="lopicker-' + item.id + '"></div>');
                            if (i < item.inputLength - 1) html.push('<div style="width:' + item.content.margin + 'px;float:left;height: 5px;"></div>');
                        }
                        html.push('<div style="clear:both;"></div>');
                        item.pickerContent.html(html.join(''));
                        ...
                        item.pickerContent.find('[data-palette-target]').each(function () {
                            ...
                            item.pickerPalette.push({
                                itemId: item.id,
                                inputIndex: idx,
                                louiInstance: new luplino.ui.palette(paletteConfig)
                            });
                        });
                    }
                };
                ...
                return function (boundID, tryCount) {
                    ...
                    this.activePickerArrow = this.activePicker.find(".lo-picker-arrow");
                    ...
                    jQuery(window).bind("resize.lopicker", function () {
                        alignPicker.call(this);
                    }.bind(this));

                    // bind key event
                    jQuery(window).bind("keyup.lopicker", function (e) {
                        e = e || window.event;
                        onBodyKeyup.call(this, e);
                        U.stopEvent(e);
                    }.bind(this));

                    jQuery(window).bind("click.lopicker", function (e) {
                        e = e || window.event;
            ...
            this.close = function (item, state) {
                ...
                jQuery(window).unbind("resize.lopicker");
                jQuery(window).unbind("click.lopicker");
                jQuery(window).unbind("keyup.lopicker");
    ...
    PICKER = luplino.ui.picker;
})();

// luplino.ui.picker.tmpl
(function () {

    var PICKER = luplino.ui.picker;
    var U = luplino.util;

    var pickerTmpl = function pickerTmpl() {
        return "\n<div class=\"lo-ui-picker {{theme}}\" id=\"{{id}}\" data-picker-els=\"root\" {{#zIndex}}style=\"z-index:{{zIndex}};\"{{/zIndex}}>\n    {{#title}}\n        <div class=\"lo-picker-heading\">{{title}}</div>\n    {{/title}}\n    <div class=\"lo-picker-body\">\n        <div class=\"lo-picker-content\" data-picker-els=\"content\" style=\"width:{{contentWidth}}px;\"></div>\n        {{#btns}}\n            <div class=\"lo-picker-buttons\">\n            {{#btns}}\n                {{#@each}}\n                <button data-picker-btn=\"{{@key}}\" class=\"btn btn-default {{@value.theme}}\">{{@value.label}}</button>\n                {{/@each}}\n            {{/btns}}\n            </div>\n        {{/btns}}\n    </div>\n    <div class=\"lo-picker-arrow\"></div>\n</div>\n";
    };
    ...
    PICKER.tmpl = {
        ...
        get: function get(tmplName, data, columnKeys) {
            return luplino.mustache.render(PICKER.tmpl[tmplName].call(this, columnKeys), data);
        }
    };
})();
/**
 * luplino.ui.picker_instance
 * @type {lopicker}
 * @example
 * ```js
 * // picker 기본 속성을 변경해야 한다면
 * luplino.ui.picker_instance.setConfig({
 * });
 *
 * ```
 */
if (luplino && luplino.ui && luplino.ui.picker) {
    luplino.ui.picker_instance = new luplino.ui.picker();

    jQuery.fn.lopicker = function () {
        return function (config) {
            if (luplino.util.isString(arguments[0])) {
                ...
                switch (methodName) {
                    case "open":
                        return luplino.ui.picker_instance.open(this);
                        break;
                    case "close":
                        return luplino.ui.picker_instance.close(this);
                        break;
                    case "setValue":
                        return luplino.ui.picker_instance.setContentValue(this, arguments[1], arguments[2]);
                        break;
                    default:
                        return this;
                }
                ...
                jQuery.each(this, function () {
                    ...
                    luplino.ui.picker_instance.bind(config);
                });
            }
            return this;
        };
